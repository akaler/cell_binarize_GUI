...
def update_binary_image(begin,end,threshold): 
    ...
    for i in range(begin,end+1,4):
        ...
        src = cv2.imread("{}.png".format(i), cv2.IMREAD_GRAYSCALE)
...
def setup_images(begin, end):
    ...
    for i in range(begin, end+1, 4):
        List_images.append(ImageTk.PhotoImage(Image.open("{}.png".format(i))))
...
def draw_grid():
    ...
    #Texts
    q1_text = Label(root, text="Frame {}".format(begin+1))
    q2_text = Label(root, text="Frame {}".format(begin+4))
    q3_text = Label(root, text="Frame {}".format(begin+8))
    q4_text = Label(root, text="Frame {}".format(begin+12)) 
...
def initial_view():
    ...
    #scale widget init
    scale_widget = Scale(root,label="Pick Threshold", orient="horizontal",length = 1295,  resolution=1, from_=0, to=254, tickinterval = 10, command=display_segmented_image)
